...
. パイプラインが保存されていないという警告が表示される場合がありますが、これは正常な動作です。この場合は、**[Save and Submit]** をクリックしてください。
+
[.bordershadow]
...
. **Job submission to Data Science Pipelines succeeded**というポップアップが表示されたら、**[Run Details]**リンクをクリックして、パイプラインの進行状況を確認します。
...
. 先ほど実行したパイプラインを見つけます。
. 行の一番最後の3つのドットをクリックし、**Create schedule**をクリックします。
+
[.bordershadow]
image::03/07-create-schedule.png[create schedule]

. 次の画面で名前を指定し、**Day(日)**ごとに実行される定期実行を選択して、**Create schedule**をクリックします。
+
[.bordershadow]
