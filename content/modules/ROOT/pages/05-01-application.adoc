...
- トグルを使用してWorkbench statusダイアログを開き、下部の**Start workbench**をクリックして再度起動してください。起動が完了(ダイアログ上部の"Workbench status"の文字の横がStoppedからRunningに変化)したら、先ほどと同様に、Workbench statusダイアログ右上のxを押して閉じてください。
