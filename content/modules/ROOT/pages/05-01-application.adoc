...
- {rhoai} ダッシュボードに戻ります。
- トグルを使用してWorkbench statusダイアログを開きます。
+
[.bordershadow]
image::05/05-toggle-off-workbench.png[]
- ** Stop workbench **を押します。ワークベンチを停止します。
+
[.bordershadow]
image::05/05-confirm-turn-off0.png[]
- 確認画面で** Stop workbench **を押し、ワークベンチを停止します。
+
[.bordershadow]
image::05/05-confirm-turn-off.png[]


- 完全に停止するには、おそらく 5 秒から 10 秒かかります。停止したら(ダイアログ上部の"Workbench status"の文字の横がRunningからStoppedに変化)、Workbench statusダイアログ右上のxを押して閉じてください。
...
- トグルを使用してWorkbench statusダイアログを開き、**Start workbench**ワークベンチをクリックして再度起動してください。起動が完了(ダイアログ上部の"Workbench status"の文字の横がStoppedからRunningに変化)したら、先ほどと同様に、Workbench statusダイアログ右上のxを押して閉じてください。
- ワークベンチが起動したら、**My Workbench** 横のリンク(四角から矢印が出ているアイコン)をクリックします。:
+
[.bordershadow]
