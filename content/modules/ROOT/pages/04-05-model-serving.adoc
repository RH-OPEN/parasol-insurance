...
. これから、モデルを {rhoai-short} モデル サービングにデプロイする必要があります。
. もう 1 つの接続(ストレージ接続設定)を作成します...
.. 既存の接続とほぼ同じ情報を使用します。
...
* データ サイエンス プロジェクトの *Connections* タブで、 **Create connection** をクリックして、モデルのコピーが保存されている共有ストレージ (Minio) を参照する接続を作成します。
+
[.bordershadow]
image::04/04-add-data-connection.png[]

* 以下の画面で **S3 compatible object storage - v1** * を選択します。
+
[.bordershadow]
...
* **Create** をクリックして、connectionを作成します。
...
* **Multi-model serving platform** タイプのモデル (右側) で、 **Select multi-model** をクリックします。
+
[.bordershadow]
image::04/add-model-server.png[]

* 遷移した先の画面の下部の**Add model server** をクリックします。入力する必要がある情報は次のとおりです:
...
* これを行うには、 *Interface endpoint* 列の *Internal endpoint details* リンクをクリックします。
